...
int http_response(struct ntyevent *ev) {
	...
	if (filefd == -1) { // return 404
		...
		if (S_ISDIR(stat_buf.st_mode)) {
			// 检查目录下是否有index.html文件
			char index_path[BUFFER_LENGTH];
			// 使用snprintf确保不会溢出缓冲区
			int path_len = snprintf(index_path, sizeof(index_path), "%s/index.html", ev->resource);
			if (path_len >= sizeof(index_path)) {
				// 路径太长，无法处理
				ev->ret_code = 403;
				ev->length = sprintf(ev->buffer, 
					...
					"Content-Type: text/html;charset=ISO-8859-1\r\n"
					"Content-Length: 93\r\n\r\n"
					"<html><head><title>403 Forbidden</title></head><body><H1>403 Directory access forbidden</H1></body></html>\r\n\r\n" );
			} else {
				// 尝试打开index.html文件
				int index_fd = open(index_path, O_RDONLY);
				if (index_fd != -1) {
					// 如果存在index.html，就使用它
					strncpy(ev->resource, index_path, BUFFER_LENGTH - 1);
					ev->resource[BUFFER_LENGTH - 1] = '\0';
					struct stat index_stat;
					fstat(index_fd, &index_stat);
					close(index_fd);
...
int main(int argc, char *argv[]) {
	...
	int i = 0;
	int sockfds[PORT_COUNT] = {0};//存放多个监听端口的sockfd
	for (i = 0;i < PORT_COUNT;i ++) {
		sockfds[i] = init_sock(port+i);//监听从port开始的连续多个端口
		ntyreactor_addlistener(reactor, sockfds[i], accept_cb);
	}//这里的for 循环，是为了监听多个端口，因为ntyreactor_addlistener函数中，ntyreactor_idx函数中，会根据sockfd来计算blkidx，从而决定放在哪个block中
